...
$('#sampleTag').addEventListener('click', async e => {
  ...
  try {
    ...
    const res = await fetch('samples/humanoid.fbx');
...
async function populateAnimList() {
  ...
  let data = null;
  try { data = await (await fetch('animations/manifest.json')).json(); } catch (_) {}
  let clips = [];
  if (Array.isArray(data)) {
    clips = data.map(x => typeof x === 'string'
      ? { label: x.replace(/\.fbx$/i, ''), path: 'animations/' + x } : x);
  ...
  clips.forEach(c => addAnimButton(c.label || c.path, c.path));
}
function addAnimButton(name, url) {
  const host = $('#extClips');
  ...
  b.innerHTML = `▶ ${name}`;
  b.addEventListener('click', () => playAnim(name, url, b));
  host.appendChild(b);
  return b;
}
async function playAnim(name, url, btn) {
  ...
  try {
    ...
    if (!file && url) { const blob = await (await fetch(url)).blob(); file = new File([blob], name + '.fbx'); loadedAnims[name] = file; }
    ...
    toast(miss
      ? `Playing “${info.name}” — ⚠ clip needs ${miss} joint(s) you don't have (see report).`
      : `Playing “${info.name}” (${info.duration}s). Wrong way? Use Facing.`, miss ? 'warn' : 'success');
...
(function () {
  ...
  window.addEventListener('message', async (e) => {
    ...
    try {
      if (d.type === 'studio:loadUrl') { await loadFromUrl(d.url, d.name); post('studio:loaded', { name: d.name }); }
