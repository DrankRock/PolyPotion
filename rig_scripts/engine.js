...
let skelHelper = null, restQuats = [], rigData = null;
...
// ---------- cameras (4 panels) ----------
const persp = { cam: null, target: new THREE.Vector3(), az: 0.6, el: 0.25, radius: 4 };
...
function updateCameras() {
  // perspective
  const r = modelR || 1;
  persp.target.copy(modelC);
  const pe = persp.el, pa = persp.az;
  persp.cam.position.set(
    modelC.x + persp.radius * Math.cos(pe) * Math.sin(pa),
    modelC.y + persp.radius * Math.sin(pe),
    modelC.z + persp.radius * Math.cos(pe) * Math.cos(pa)
  );
...
function frameAll() {
  persp.radius = modelR * 3.2; persp.az = 0.6; persp.el = 0.22;
...
function bindPointer() {
  ...
  canvas.addEventListener('pointerdown', e => {
    ...
    if (hit) {
      ...
      drag = { mode: 'joint', joint: hit, panel: p, plane };
    } else if (p.kind === 'persp') {
      drag = { mode: 'orbit', panel: p, lx: px, ly: py };
    } else {
  ...
  const up = e => {
    ...
    if (drag && !moved && drag.mode !== 'joint') {
      // click empty space → keep selection; click on joint handled at down
    }
...
function clearRig() {
  ...
  restQuats = []; rigData = null; animPlaying = false;
...
// ---- FAST weights (bone-segment distance, with cross-limb cutoff) ----
...
function bindHQ(meshData, segs, opts, onP) {
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker('rig_scripts/skin-worker.js?v=' + Date.now());
...
function restPose() {
  ...
  stopExternal();
  if (skeleton) skeleton.bones.forEach((b, i) => { if (restQuats[i]) b.quaternion.copy(restQuats[i]); });
}
function stopExternal() { extActive = false; if (extAction) { extAction.stop(); } }
...
function poseSkeleton(t) {
  ...
  skeleton.bones.forEach((b, i) => { if (restQuats[i]) b.quaternion.copy(restQuats[i]); });
...
function mapSrcToRig(name) {
  let n = name.replace(/^mixamorig:?/i, '');
  let side = null;
  if (/^left/i.test(n)) side = 'L'; else if (/^right/i.test(n)) side = 'R';
  const core = n.replace(/^(left|right)/i, '').toLowerCase();
  const M = {
    hips: 'Hips', pelvis: 'Hips', spine: 'Spine', spine1: 'Chest', spine01: 'Chest',
    spine2: 'Chest', spine02: 'Chest', chest: 'Chest', neck: 'Neck', neck1: 'Neck',
    head: 'Head',
    arm: 'Shoulder', upperarm: 'Shoulder', forearm: 'Elbow', lowerarm: 'Elbow', hand: 'Wrist',
    upleg: 'UpperLeg', thigh: 'UpperLeg', leg: 'Knee', calf: 'Knee', shin: 'Knee',
    foot: 'Ankle', toebase: 'Toe', toe: 'Toe', ball: 'Toe',
    handthumb1: 'Thumb', thumb1: 'Thumb', handindex1: 'Index', index1: 'Index',
    handmiddle1: 'Middle', middle1: 'Middle', handring1: 'Ring', ring1: 'Ring',
    handpinky1: 'Pinky', pinky1: 'Pinky',
  };
  const base = M[core];
  if (!base) return null;
  const CENTER = ['Hips', 'Spine', 'Chest', 'Neck', 'Head'];
...
E.loadExternalClip = async function (file) {
  ...
  // Reference pose = the clip's FIRST FRAME (not the source bind pose). This maps
  // frame 0 onto OUR T-pose rest, so differing rest orientations cancel and the
  // motion is applied relatively — avoids arms flipping when the source's bind
  // pose differs from how the user placed the joints.
  mixer.setTime(0); src.updateMatrixWorld(true);
  const restWorld = {};
  ...
  extState = { name: file.name.replace(/\.fbx$/i, ''), times, deltas, duration: times[frames - 1] };
  ...
  return { name: extState.name, duration: +extState.duration.toFixed(2), frames, report };
};
...
function buildExternalClip(yawDeg) {
  ...
  for (let f = 0; f < frames; f++) {
    ...
    for (let bi = 0; bi < bones.length; bi++) {
      ...
      let myWorld;
      const d = frame[bone.name];
      if (d) {
        // re-orient the motion frame by yaw: yaw · delta · yaw⁻¹
        myWorld = yawQ.clone().multiply(d).multiply(yawInv);
      } else {
